...
class ReviewManager(models.Manager):
	...
	def createReview(self, reviewInfo):

		print reviewInfo

		authorInfo = reviewInfo['addauthor']
		if len(Author.objects.filter(author = authorInfo)) == 0:  #If author doesnt exist,
		...
		Review.objects.create(review=reviewInfo['bookreview'], ratings= reviewInfo['rating'], user=User.objects.get(id = 1), book = Book.objects.get(title=reviewInfo['book_title']))
